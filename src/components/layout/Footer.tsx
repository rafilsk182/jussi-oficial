...
const LogoJussi = () => (
  <svg width="157" height="55" viewBox="0 0 94 33" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  return (
    <div
      ...
      <svg width="12" height="12" viewBox="0 0 14 14" fill="none">
        <path d="M3 7H11M11 7L8 4M11 7L8 10"
          stroke={hovered ? '#070707' : 'white'}
...
export function Footer() {
  return (
    <footer id="footer" className="w-full" style={{ backgroundColor: '#070707', borderTop: '2px solid rgba(255,255,255,0.2)' }}>

      {/* ═══════════ DESKTOP ═══════════ */}
      <div className="hidden lg:block" style={{ overflow: 'hidden' }}>
        <div style={{ position: 'relative', height: '320px', maxWidth: '1366px', margin: '0 auto' }}>

          {/* Line 3 — 26.35% */}
          <div style={{ position: 'absolute', left: '26.35%', top: 0, bottom: 0, width: '2px', backgroundColor: 'rgba(255,255,255,0.2)', zIndex: 1 }} />
          {/* Line 4 — 43.85% */}
          <div style={{ position: 'absolute', left: '43.85%', top: 0, bottom: 0, width: '2px', backgroundColor: 'rgba(255,255,255,0.2)', zIndex: 1 }} />

          {/* Círculo escritório */}
          <div style={{ position: 'absolute', left: '62%', top: '50%', transform: 'translate(-50%, -50%)', width: '380px', height: '380px', borderRadius: '50%', border: '1px solid rgba(255,255,255,0.15)', zIndex: 0, pointerEvents: 'none' }} />
          {/* Círculo contato */}
          <div style={{ position: 'absolute', left: '93%', top: '50%', transform: 'translate(-50%, -50%)', width: '380px', height: '380px', borderRadius: '50%', border: '1px solid rgba(255,255,255,0.15)', zIndex: 0, pointerEvents: 'none' }} />

          {/* Grid 3 colunas */}
          <div style={{ position: 'absolute', inset: 0, display: 'grid', gridTemplateColumns: '26.35% 17.5% 1fr', zIndex: 2 }}>

            {/* Col 1: Logo + Social */}
            <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: '28px', paddingLeft: '64px' }}>
              <Link href="/"><LogoJussi /></Link>
              <div style={{ display: 'flex', gap: '10px' }}>
                {SOCIAL.map((s, i) => (
                  <a key={i} href={s.href} target="_blank" rel="noopener noreferrer"
                    style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '38px', height: '38px', borderRadius: '50%', border: '1px solid rgba(255,255,255,0.35)', flexShrink: 0 }}>
                    <img src={s.icon} alt={s.label} style={{ width: '16px', height: '16px', filter: 'brightness(0) invert(1)' }} />
                  </a>
                ))}
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '13px' }}>
              {NAV_LINKS.map((link, i) => (
                <a key={i} href={link.href}
                  style={{ fontFamily: 'Barlow, sans-serif', fontSize: '12px', fontWeight: 400, color: '#ffffff', textDecoration: 'none', textTransform: 'uppercase', letterSpacing: '0.1em', transition: 'color 250ms ease', whiteSpace: 'nowrap' }}
                  onMouseEnter={e => (e.currentTarget.style.color = '#00ffa6')}
            ...
            {/* Col 3: Cards */}
            <div style={{ display: 'flex', gap: '16px', alignItems: 'center', padding: '0 32px' }}>

              {/* Card ESCRITÓRIO */}
              <div style={{ flex: 1, height: '174px', border: '1px solid rgba(195,195,195,0.45)', borderRadius: '16px', padding: '28px 28px', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', gap: '14px', backgroundColor: '#070707' }}>
                <h4 style={{ fontFamily: 'Barlow, sans-serif', fontSize: '18px', fontWeight: 600, color: '#ffffff', margin: 0, textTransform: 'uppercase', letterSpacing: '0.06em' }}>ESCRITÓRIO</h4>
                <p style={{ fontFamily: 'Barlow, sans-serif', fontSize: '13px', color: '#c3c3c3', margin: 0, lineHeight: 1.6 }}>
                  WPP CAMPUS<br />Av. Mofarrej, 971 – Vila Leopoldina, São Paulo – SP
                </p>
              </div>
              ...
              <div style={{ flex: 1, height: '174px', border: '1px solid rgba(195,195,195,0.45)', borderRadius: '16px', padding: '28px 28px', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', gap: '14px', backgroundColor: '#070707' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <h4 style={{ fontFamily: 'Barlow, sans-serif', fontSize: '18px', fontWeight: 600, color: '#ffffff', margin: 0, textTransform: 'uppercase', letterSpacing: '0.06em' }}>CONTATO</h4>
                  <a href="#contact" style={{ textDecoration: 'none' }}><ArrowButton /></a>
                </div>
                <p style={{ fontFamily: 'Barlow, sans-serif', fontSize: '13px', color: '#c3c3c3', margin: 0, lineHeight: 1.6 }}>
      ...
      {/* ═══════════ MOBILE ═══════════ */}
      <div className="lg:hidden" style={{ backgroundColor: '#070707' }}>

        {/* Logo centralizado */}
        <div style={{ display: 'flex', justifyContent: 'center', padding: '48px 24px 24px' }}>
          <Link href="/"><LogoJussi /></Link>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '12px', paddingBottom: '40px' }}>
          {SOCIAL.map((s, i) => (
            <a key={i} href={s.href} target="_blank" rel="noopener noreferrer"
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '40px', height: '40px', borderRadius: '50%', border: '1px solid rgba(255,255,255,0.35)' }}>
              <img src={s.icon} alt={s.label} style={{ width: '18px', height: '18px', filter: 'brightness(0) invert(1)' }} />
            </a>
          ))}
        </div>

        {/* Linha divisória */}
        <div style={{ width: '100%', height: '2px', backgroundColor: 'rgba(255,255,255,0.2)' }} />

        {/* Nav links centralizados */}
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '20px', padding: '36px 24px' }}>
          {NAV_LINKS.map((link, i) => (
            <a key={i} href={link.href}
              style={{ fontFamily: 'Barlow, sans-serif', fontSize: '13px', fontWeight: 400, color: '#ffffff', textDecoration: 'none', textTransform: 'uppercase', letterSpacing: '0.1em' }}>
              {link.label}
            </a>
          ))}
        </div>
        ...
        {/* Cards com círculos atrás */}
        <div style={{ position: 'relative', overflow: 'hidden', padding: '32px 24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>

          {/* Wrapper escritório: círculo FORA do card, atrás dele */}
          <div style={{ position: 'relative' }}>
            <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '360px', height: '360px', borderRadius: '50%', border: '1px solid rgba(255,255,255,0.18)', pointerEvents: 'none', zIndex: 0 }} />
            <div style={{ position: 'relative', zIndex: 1, border: '1px solid rgba(195,195,195,0.45)', borderRadius: '16px', padding: '24px 20px', backgroundColor: '#070707', display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <h4 style={{ fontFamily: 'Barlow, sans-serif', fontSize: '18px', fontWeight: 600, color: '#ffffff', margin: 0, textTransform: 'uppercase' }}>ESCRITÓRIO</h4>
              <p style={{ fontFamily: 'Barlow, sans-serif', fontSize: '13px', color: '#c3c3c3', margin: 0, lineHeight: 1.6 }}>
                WPP CAMPUS<br />Av. Mofarrej, 971 – Vila Leopoldina, São Paulo – SP
              </p>
            </div>
          ...
          {/* Wrapper contato: círculo FORA do card, atrás dele */}
          <div style={{ position: 'relative' }}>
            <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '360px', height: '360px', borderRadius: '50%', border: '1px solid rgba(255,255,255,0.18)', pointerEvents: 'none', zIndex: 0 }} />
            <div style={{ position: 'relative', zIndex: 1, border: '1px solid rgba(195,195,195,0.45)', borderRadius: '16px', padding: '24px 20px', backgroundColor: '#070707', display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <h4 style={{ fontFamily: 'Barlow, sans-serif', fontSize: '18px', fontWeight: 600, color: '#ffffff', margin: 0, textTransform: 'uppercase' }}>CONTATO</h4>
                <a href="#contact" style={{ textDecoration: 'none' }}><ArrowButton /></a>
              </div>
              <p style={{ fontFamily: 'Barlow, sans-serif', fontSize: '13px', color: '#c3c3c3', margin: 0, lineHeight: 1.6 }}>
                Deixe sua mensagem que responderemos em breve.
      ...
      {/* Barra inferior */}
      <div style={{ backgroundColor: '#232323', borderTop: '2px solid rgba(255,255,255,0.2)' }}>
        <div style={{ maxWidth: '1366px', margin: '0 auto', padding: '18px 64px' }}>
          <div className="flex flex-col lg:flex-row items-center justify-between gap-4">
            <div style={{ display: 'flex', gap: '32px' }}>
              <Link href="/privacidade" style={{ fontFamily: 'Barlow, sans-serif', fontSize: '12px', color: '#c3c3c3', textDecoration: 'none', textTransform: 'uppercase', letterSpacing: '0.1em' }}>
                Política de Privacidade
              </Link>
              <Link href="/cookies" style={{ fontFamily: 'Barlow, sans-serif', fontSize: '12px', color: '#c3c3c3', textDecoration: 'none', textTransform: 'uppercase', letterSpacing: '0.1em' }}>
                Cookies
              </Link>
            </div>
